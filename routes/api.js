...
// Вспомогательная функция загрузки (GET/POST)
...
// Универсальный парсер списка фильмов (каталог, категория, поиск)
function parseMoviesList(html) {
  ...
  $('.base.shortstory').each((i, el) => {
    ...
    if ($titleLink.length === 0) $titleLink = $card.find('h3.btl a').first();
